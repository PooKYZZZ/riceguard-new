...
if not MONGO_URI:
    raise ValueError(
        ...
        "Please configure MongoDB Atlas connection in backend/.env file. "
...
if MONGO_URI == "mongodb://localhost:27017":
    raise ValueError(
        "MongoDB localhost connection detected. RiceGuard requires MongoDB Atlas (cloud). "
        "Please update MONGO_URI in backend/.env with your MongoDB Atlas connection string. "
        "Format: mongodb+srv://<username>:<password>@<cluster>.mongodb.net/riceguard_db"
    )

if "REPLACE_USERNAME" in MONGO_URI or "REPLACE_PASSWORD" in MONGO_URI or "REPLACE_CLUSTER" in MONGO_URI:
    raise ValueError(
        "MongoDB Atlas placeholders detected in MONGO_URI. "
        "Please replace REPLACE_USERNAME, REPLACE_PASSWORD, and REPLACE_CLUSTER "
        "with your actual MongoDB Atlas credentials in backend/.env file. "

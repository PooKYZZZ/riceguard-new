...
# SECURITY: Pre-generate constant dummy hash for timing attack protection
# This is used when user doesn't exist to maintain consistent timing
# NOTE: For educational capstone project - production systems should use
# more sophisticated constant-time comparison techniques
_DUMMY_PASSWORD_HASH = hash_password("dummy_password_for_timing_protection_x9K2mP8qL4vN3wE7sR1tY6uI0oA5zX")
...
def login(body: LoginIn, response: Response) -> LoginOut:
    ...
    if user:
        ...
        # No user found - use constant dummy hash to prevent timing attacks
...
# ============================ USER PROFILE ==================== #
@router.get("/user/profile", response_model=UserProfileOut, tags=["user"])
def get_user_profile(creds: HTTPAuthorizationCredentials = Depends(bearer)) -> UserProfileOut:
    """Get current user's profile information."""
    claims = require_user(creds)
    user_id = claims.sub
    db: Any = get_db()
    
    user = db.users.find_one({"_id": as_object_id(user_id)})
    if not user:
    ...
    return UserProfileOut(
        ...
        updatedAt=user.get("updatedAt", user["createdAt"])
    )

@router.put("/user/profile", response_model=UserProfileOut, tags=["user"])
def update_user_profile(
    ...
    if avatar:
        # CRITICAL: Validate file type on server
        allowed_mime_types = [
        ...
        # Save avatar with secure filename
        ensure_upload_dir()
        avatar_path = save_upload(avatar, prefix="avatar_")
    ...
    # Fetch updated user
    updated_user = db.users.find_one({"_id": as_object_id(user_id)})
    
    log.info(f"User profile updated: {updated_user['email']}")
    ...
    return UserProfileOut(
        ...
        updatedAt=updated_user.get("updatedAt", updated_user["createdAt"])
    )

@router.put("/user/avatar", response_model=dict, tags=["user"])
def update_avatar_only(
    avatar: UploadFile = File(...),
    creds: HTTPAuthorizationCredentials = Depends(bearer)
) -> dict:
    """Update only the user's avatar image."""
    claims = require_user(creds)
    user_id = claims.sub
    db: Any = get_db()
    ...
    db.users.update_one(
        {"_id": as_object_id(user_id)},
        {"$set": {"avatar": avatar_path, "updatedAt": datetime.now(timezone.utc)}}
    )
    
    log.info(f"Avatar updated for user: {user_id}")

...
def _validate_model_path(path_str: str) -> str:
    ...
    try:
        ...
        # SECURITY: Allow legitimate relative paths (like ../ml/) but validate after resolution
        # The actual security check happens below when we verify the resolved path is within allowed directories

        # Allow absolute paths on Windows (contains :) or Unix (starts with /)
        if not (':' in path_str or path_str.startswith('/')):
            # Only reject relative paths that start with / on Unix
        ...
        allowed_dirs = [
            repo_root / "ml",
            repo_root / "src" / "ml",  # New src structure
            repo_root / "models",

        ...
        if dirs_ok and files_ok:
            ...
            if not dirs_ok:
                ...
                if not BACKEND_DIR.exists():
                    safe_print("  - src/backend/")
                if not FRONTEND_DIR.exists():
                    safe_print("  - src/frontend/")
                if not (BACKEND_DIR / ".venv").exists():
                    safe_print("  - src/backend/.venv (run setup.py)")
                if not (FRONTEND_DIR / "node_modules").exists():
                    safe_print("  - src/frontend/node_modules (run setup.py)")
            if not files_ok:
                safe_print("Missing files:")
                if not (BACKEND_DIR / ".env").exists():
                    safe_print("  - src/backend/.env (run setup.py)")
                if not (FRONTEND_DIR / ".env").exists():
                    safe_print("  - src/frontend/.env (run setup.py)")
            return False

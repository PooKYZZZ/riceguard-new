...
const ProfilePanel = React.memo(({ isOpen, onClose, user }) => {
  ...
  // Initialize form data when user prop changes - optimized with useCallback
  ...
  // Memory leak prevention: cleanup avatar preview URL
  useEffect(() => {
    return () => {
  ...
  }, [isEditMode, originalData, user.avatar]);
  ...
  const validateAvatarFile = useCallback((file) => {
    // Validate file type
  ...
  const handleAvatarFile = useCallback((file) => {
    ...
    // Create preview with cleanup for previous blob URL
    if (avatarPreview && avatarPreview.startsWith('blob:')) {
      URL.revokeObjectURL(avatarPreview);
    }
  ...
  }, [validateAvatarFile, avatarPreview]);
  ...
  }, [validateForm, formData, avatarFile]);

  // Memoize character count text
  const characterCountText = useMemo(() => {
    return `${formData.bio.length}/200 characters`;
  }, [formData.bio]);
